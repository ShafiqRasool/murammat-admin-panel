    ...
    const load = async () => {
      try {
        const [cities, categories, services, providers] = await Promise.all([
          getCities(), getCategories(), getServices(), getProviders(),
        ]);
        setStats({
          ...
          pendingProviders: providers.filter(p => p.approval_status === 'unapproved').length,
          approvedProviders: providers.filter(p => p.approval_status === 'approved').length,
        });
      } catch {
